...
class LogSendReviewResult
{
    ...
    public function handle(SendReviewResult $event)
    {
        ...
        $paperEventLog->type = Config::get('constants.PAPER_EVENT.SEND_REVIEW_RESULT');
        $paperEventLog->message = sprintf(Config::get('constants.PAPER_EVENT.SEND_REVIEW_RESULT_MESSAGE'), Auth::user()->full_name, $event->reviewAssignment->paper->title, Carbon::now()->format('H:i d/m/Y'));
        $paperEventLog->save();
    }
}
